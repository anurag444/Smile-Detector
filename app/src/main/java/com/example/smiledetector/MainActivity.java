...
public class MainActivity extends AppCompatActivity implements FrameProcessor {
    ...
    //FrameProcessor helps to retrieve images , sizing images which we have to do manually
    //Frame class helps us to frame everything that shows up at UI4
    ...
    private ArrayList<face_detection_model> faceModelList;
...
//        Button toggle=findViewById(R.id.toggle_button);
        ...
        // Set up cameraview from the library
...
//        });
        ...
        bottomSheetRecycler.setAdapter(new FaceDetectionAdapter(faceModelList,MainActivity.this));
    ...
    private void analyseImage(final Bitmap bitmap) {
        ...
        //set options which we want on the image
        ...
        faceDetector.detectInImage(firebaseVisionImage)
                .addOnSuccessListener(new OnSuccessListener<List<FirebaseVisionFace>>() {
                    ...
                    public void onSuccess(List<FirebaseVisionFace> firebaseVisionFaces) {
                        //it is just a paper, so later we can put the info like faces, eyes.
                        // It is mutuable so that it can be changed when we actually detect faces
                        Bitmap mutableImage = bitmap.copy(Bitmap.Config.ARGB_8888, true);

                        detectFaces(firebaseVisionFaces, mutableImage);

                        //this will show the image after the result
    ...
    private void detectFaces(List<FirebaseVisionFace> firebaseVisionFaces, Bitmap bitmap) {
        ...
        for (int i = 0; i < firebaseVisionFaces.size(); i++) {
            ...
            //moving the text to the top of the square
            ...
            //After drawing on the face, we add items to the recycler view with probabilities
            faceModelList.add(new face_detection_model(i, "Smiling Probability " + face.getSmilingProbability()));
            faceModelList.add(new face_detection_model(i, "Left Eye Open Probability " + face.getLeftEyeOpenProbability()));
            faceModelList.add(new face_detection_model(i, "Right Eye Open Probability " + face.getRightEyeOpenProbability()));

            if (face.getSmilingProbability()>0.5) {
                faceModelList.add(new face_detection_model(i,"The Person is smiling"));
            }else{
                faceModelList.add(new face_detection_model(i, "The Person is Not smiling"));
    ...
    public void process(@NonNull Frame frame) {
        ...
        FirebaseVisionImageMetadata metadata = new FirebaseVisionImageMetadata
                ...
                .setRotation((cameraFacing == Facing.FRONT)//check if phone is in potrait or vertical
                    ...
                    public void onSuccess(List<FirebaseVisionFace> firebaseVisionFaces) {
                        ...
                        for (FirebaseVisionFace face : firebaseVisionFaces) {
                            ...
                            if (cameraFacing == Facing.FRONT) {
                                //Flip image! or rotate the image
